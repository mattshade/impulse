jQuery( document ).ready(function( $ ) {
	...
	// Dependency for login redirect url option
	$( document ).ready( function(){
		var radio_options = $( 'input[data-depend-id="login-redirect-url"]' );

		$.each( radio_options, function( i, val ){
			if( val.checked && val.value === 'custom' ) {
				$( '#custom-redirect-url' ).closest( 'tr' ).fadeIn();
				return false;
			} else {
				$( '#custom-redirect-url' ).closest( 'tr' ).fadeOut();
			}
		} );


		//will trigger condition on click will hide or show the custom redirect url
		radio_options.on( 'click', function(){
			if( $(this).val() === 'custom' ) {
				$( '#custom-redirect-url' ).closest( 'tr' ).fadeIn();
			} else {
				$( '#custom-redirect-url' ).closest( 'tr' ).fadeOut();
