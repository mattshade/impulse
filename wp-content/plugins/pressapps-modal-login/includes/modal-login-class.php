...
class PAML_Class {
	...
	public function paml_add_nav_menu_metabox() {
		add_meta_box( 'paml_metabox_modal_link', __( 'PA Modal Link', 'pressapps' ), array(
			$this,
	...
	public function paml_resources() {
		...
		switch ( $paml_options['login-redirect-url'] ) {
			case 'home':
				$login_url = home_url();
				break;
			case 'custom':
				$login_url = filter_var( $paml_options['custom-redirect-url'], FILTER_VALIDATE_URL ) ? $paml_options['custom-redirect-url'] : $_SERVER['REQUEST_URI'];
				break;
			case 'current':
			default:
		...
		$registration_url = filter_var( $paml_options['registration-redirect-url'], FILTER_VALIDATE_URL ) ? $paml_options['registration-redirect-url'] : $_SERVER['REQUEST_URI'];
		...
		if ( ! is_user_logged_in() ) {
			wp_localize_script( 'paml-script', 'modal_login_script', array(
				...
				'ajax_spinner_color'    => $paml_options['spinner-color']
			) );
			// If user is not logged in and google captcha key are assigned, load Google captcha's api
			if(isset($paml_options['google_captcha_sitekey'])){
	...
	public function paml_print_style_css() {
		...
		if ( $paml_options['font-color'] ) {
			echo '#modal-login, #modal-login h2, .ml-content > a.ml-close-btn { color: ' . sanitize_text_field( $paml_options['font-color'] ) . "}\n";
		}
	...
	public function paml_ajax_login() {
		...
		elseif ( isset( $_REQUEST['register'] ) ) {
			...
			if(isset($paml_options['google_captcha_sitekey']) && $paml_options['google_captcha_sitekey'] != ""){
				$recaptcha = $_REQUEST['recaptcha'];
				$google_url = "https://www.google.com/recaptcha/api/siteverify";
				$secret = $paml_options['google_captcha_secretkey'];
			...
			$user_data = array();
			if(isset($paml_options['userdefine_fullname'])){
	...
	public function paml_register_new_user( $user_information ) {
		...
		if(isset($paml_options['userdefine_fullname'])){
			...
			if ( $sanitized_First_Name == '' ) {
				$errors->add( 'empty_firstname', __( 'Please enter a First Name.', 'pressapps' ) );
			} elseif ( !validate_username( $user_information['first_name'] ) ) {
				$errors->add( 'invalid_firstname', __( 'The First Name is invalid because it uses illegal characters. Please enter a valid username.', 'pressapps' ) );
				$sanitized_First_Name = '';
			}
			// Check the lastname was sanitized
			if ( $sanitized_Last_Name == '' ) {
				$errors->add( 'empty_lastname', __( 'Please enter a Last Name.', 'pressapps' ) );
			} elseif ( !validate_username( $user_information['last_name'] ) ) {
				$errors->add( 'invalid_lastname', __( 'The Last Name is invalid because it uses illegal characters. Please enter a valid username.', 'pressapps' ) );
				$sanitized_Last_Name = '';
			}
		}
					...
					<div class="section-container">
							...
							<form action="login" method="post" id="form" class="group" name="loginform">
								...
								<p class="mluser">
									<label class="field-titles"
									       for="login_user"><?php _e( 'Username', 'pressapps' ); ?></label>
								...
								<p class="mlpsw">
									<label class="field-titles"
									       for="login_pass"><?php _e( 'Password', 'pressapps' ); ?></label>
								...
								<p id="forgetmenot">
									<label class="forgetmenot-label" for="rememberme"><input name="rememberme"
									                                                         ...
									                                                         value="forever"/> <?php _e( 'Remember Me', 'pressapps' ); ?>
									</label>
								</p>
						...
						<?php if ( ( get_option( 'users_can_register' ) && ! is_multisite() ) || ( $multisite_reg == 'all' || $multisite_reg == 'blog' || $multisite_reg == 'user' ) ) : ?>
							<div id="register" class="modal-login-content" style="display:none;">
								...
								<form action="register" method="post" id="form" class="group" name="loginform">
									...
									<?php if(isset($paml_options['userdefine_fullname'])){ ?>
										<p class="mlfirst">
											<label class="field-titles" for="reg_firstname"><?php _e( 'First Name', 'pressapps' ); ?></label>
											<input type="text" name="first_name" id="reg_firstname" class="input" placeholder="<?php if ( $labels == 'placeholders' ) { _e( 'First', 'pressapps' ); } ?>"  />
										</p>
										<p class="mllast">
											<label class="field-titles" for="reg_lastname"><?php _e( 'Last Name', 'pressapps' ); ?></label>
											<input type="text" name="last_name" id="reg_lastname" class="input" placeholder="<?php if ( $labels == 'placeholders' ) { _e( 'Last', 'pressapps' ); } ?>"  />
										</p>
									<?php } ?>
									...
									<p class="mlemail">
										<label class="field-titles"
										       for="reg_email"><?php _e( 'Email', 'pressapps' ); ?></label>
									...
									if ( isset( $paml_options['userdefine_password'] ) ) {
										...
										<p class="mlregpsw">
											<label class="field-titles"
											       for="reg_password"><?php _e( 'Password', 'pressapps' ); ?></label>
										...
										<p class="mlregpswconf">
											<label class="field-titles"
											       for="reg_cpassword"><?php _e( 'Confirm Password', 'pressapps' ); ?></label>
									...
									<?php if(isset($paml_options['google_captcha_sitekey']) && $paml_options['google_captcha_sitekey'] != ""){ ?>
										<div class="g-recaptcha" data-sitekey="<?php echo $paml_options['google_captcha_sitekey']; ?>" style="display:block;"></div>
									<?php } ?>
									...
									<p class="submit">
										...
										<input type="submit" name="user-sumbit" id="user-submit"
										       class="button button-primary button-large"
										       value="<?php esc_attr_e( 'Sign Up', 'pressapps' ); ?>"/>
						...
						<div id="forgotten" class="modal-login-content" style="display:none;">
							...
							<form action="forgotten" method="post" id="form" class="group" name="loginform">
								...
								<p class="mlforgt">
									<label class="field-titles"
									       for="forgot_login"><?php _e( 'Username or Email', 'pressapps' ); ?></label>
								...
								<p class="submit">
									...
									<input type="submit" name="user-submit" id="user-submit"
									       class="button button-primary button-large"
									       value="<?php esc_attr_e( 'Reset Password', 'pressapps' ); ?>">
			...
			<div id="tabs-panel-paml-modal-link" class="tabs-panel tabs-panel-active">
				<ul id="paml-modal-link-checklist" class="categorychecklist form-no-clear">
					<li>
						<label class="menu-item-title">
							<input type="checkbox" class="menu-item-checkbox" name="menu-item[-1][menu-item-object-id]"
							       value="-1"> <?php _e( 'Login', 'pressapps' ); ?>
							/ <?php _e( 'Logout', 'pressapps' ); ?>
						</label>
						<input type="hidden" class="menu-item-type" name="menu-item[-1][menu-item-type]" value="custom">
						<input type="hidden" class="menu-item-title" name="menu-item[-1][menu-item-title]"
						       value="<?php _e( 'Login', 'pressapps' ); ?> // <?php _e( 'Logout', 'pressapps' ); ?>">
					...
					<li>
						<label class="menu-item-title">
							<input type="checkbox" class="menu-item-checkbox" name="menu-item[-1][menu-item-object-id]"
							       value="-1"> <?php _e( 'Register', 'pressapps' ); ?>
						</label>
						<input type="hidden" class="menu-item-type" name="menu-item[-1][menu-item-type]" value="custom">
						<input type="hidden" class="menu-item-title" name="menu-item[-1][menu-item-title]"
						       value="<?php _e( 'Register', 'pressapps' ); ?>">
						<input type="hidden" class="menu-item-url" name="menu-item[-1][menu-item-url]"
