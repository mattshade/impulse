...
final class Order_Setup {
	...
	public function __construct() {
		...
		add_action( 'wcpl_process_package_for_job_listing', array( $this, 'default_to_claim_for_user_package' ), 10, 3 );
	}
	...
	public function order_created( $order_id ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			if ( $product->is_type( array( 'job_package', 'job_package_subscription' ) ) && $order->get_user_id() && isset( $item['claim_id'] ) ) {
			...
			if ( $product && $product->is_type( array( 'job_package', 'job_package_subscription' ) ) && $order->get_user_id() ) {
				...
				if ( 'yes' === get_post_meta( $product->get_id(), '_use_for_claims', true ) && isset( $item['claim_id'] ) &&  $item['claim_id'] ) {
					...
					if ( $user_package ) {
						...
						wc_paid_listings_increase_package_count( $order->get_user_id(), $user_package->id );
