...
class Listify_Ratings extends Listify_Integration {
	...
	public function setup_actions() {
		...
		/* Add rating when comment is approved */
		add_action( 'wp_set_comment_status', array( $this, 'update_comment_status_update_post_rating' ), 10, 2 );
	...
	public function add_rating_field() {
		...
		/* Only for logged in user and not post author */
...
<?php
	...
	public function save_comment_rating( $comment_id, $comment_approved ) {
		...
		/* Author cannot rate their own listing */
		$user_id = $comment->user_id; // user ID who submit comment
		$post_author = get_post_field( 'post_author', $post_id ); // post author ID
		if ( $user_id && $user_id == $post_author ) {
			return $comment_id;
		}
	 ...
	 *
	 * @param int    $comment_id
	 * @param string $comment_status new status of comment
	 */
	public function update_comment_status_update_post_rating( $comment_id, $comment_status ) {

		/* Bail if do not have rating */
	...
	public function update_post_ratings_data( $comment_id, $post_id, $rating, $action = 'add' ) {
		...
		if ( ! $ratings_data || ! is_array( $ratings_data ) || 'recalculate' == $action ) {
			...
			$args = array(
				'post_id'        => $post_id,
				'author__not_in' => get_post_field( 'post_author', $post_id ),
				'status'         => 'approve',
				'parent'         => 0,
				'meta_key'       => 'rating',
			);
