...
class Listify_Widget_Taxonomy_Image_Grid extends Listify_Widget {
	...
	function widget( $args, $instance ) {
		...
		$after_title   = '<h2 class="home-widget-description">' . $description . '</h2>' . $after_title; // add description before title closed
		$taxonomy      = $instance['taxonomy'];
		$limit         = $instance['limit'];
		$terms_include = array_filter( array_map( 'trim', explode( ',', $instance['terms'] ) ) ); // make array
		...
		$get_terms_args = array(
			...
			'number'   => $limit,
			'order'    => $order,
			'orderby'  => in_array( $orderby, array( 'name', 'count' ) ) ? $orderby : 'name',
		);
