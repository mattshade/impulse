...
class Listify_Astoundify_Favorites extends Listify_Integration {
	...
	public function setup_actions() {
		...
		if ( ! is_user_logged_in() ) {
			// Add a login form in the footer when logged out.
			add_action( 'wp_footer', array( $this, 'login_form' ) );

			// Enqueue script for guests.
...
<?php
	...
	/**
	 * Output a login form when logged out.
	 *
	 * @since 2.0.0
	 */
...
<div id="add-favorite" class="popup add-favorite-loginform-popup">

	<h2 class="popup-title"><?php esc_html_e( 'Login to Favorite', 'listify' ); ?></h2>

	<?php listify_login_form( 'add-favorite-loginform' ); ?>
...
<?php
	...
	/**
	 * Enqueue a helper script for guests.
	 *
	 * @since 2.0.0
	 */
	public function enqueue_scripts() {
		wp_enqueue_script( 'listify-astoundify-favorites', self::get_url() . 'js/astoundify-favorites.js', array( 'listify' ), 20170606, true );
	}
